  ...
  <div class="menu">
    <header>
      ...
      <div class="fr user-name">
        ...
        <span>{{username}}</span>
        <i class="icon fa-sign-out" style="cursor: pointer" @click="exitLogin()"></i>
      </div>
    </header>
    <!--菜单-->
    <div class="menu-div">
      <ul class="menu-ul">
        ...
        <li v-for="item in menuData" @click="collapseMenu(item,$event)" :id="item.id">
          ...
          <ul v-if="item.children" style="margin: 0px;" class="hide" @click="activeUl($event)">
            <li v-for="children in item.children" :id="children.id">
            ...
            children: [
              {
                ...
                icon: '',
                className: 'all-data-monitoring aa',
                id: 'allDataMonitoring',
              }
              ,
              {
                name: '数据库监控',
...
//          }
    ...
    updated() {
      ...
      if (!Utils.isEmptyObject(url)) {
        ...
        } else if (routerNam == 'dataMonitoring' || routerNam == 'DataMonitoringTable') {

//          $('ul.menu-ul a.data-monitoring').find('.yfsl-menu-arrow').css({
//              'display':'inline-block',
//              'transform':'rotate(90deg)'
//          })
          $('ul.menu-ul a.data-monitoring').siblings().removeClass('hide')
          $('ul.menu-ul a.data-monitoring').siblings().addClass('show')
        }
        $('ul.menu-ul a.data-monitoring').removeClass('router-link-exact-active router-link-active ');
      }
...
//      console.log(userName);
    ...
    methods: {
      ...
      collapseMenu: function (data, e) {
        if (data.children) {
//          $(document).find('a').removeClass('router-link-exact-active router-link-active')
          $('#dataMonitoring').find('a').removeClass('router-link-exact-active router-link-active')
          $('#dataMonitoring>a').find('.yfsl-menu-arrow').css({
            "display": "inline-block",

          })

          $('#dataMonitoring>a').find('.yfsl-menu-arrow').toggleClass('rotate')
//          $('#dataMonitoring>a').find('.yfsl-menu-arrow').toggleClass('norotate')
...
//          }
          ...
          $('#dataMonitoringTable').parent('ul').toggleClass('hide')
          $('#dataMonitoringTable').parent('ul').toggleClass('show')

//          $(e).addClass('open')
        } else {
          $('#dataMonitoring').find('ul').addClass('hide')
          $('#dataMonitoring>a').find('.yfsl-menu-arrow').removeClass('rotate')
          $('#dataMonitoring>a').find('.yfsl-menu-arrow').css("display", 'none')
        }
      },
//      collapseMenu:function(data,e){
//        if(data.children){
